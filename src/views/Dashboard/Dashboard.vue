          ...
          <div class="flex items-center px-5 py-6 bg-white rounded-md shadow-sm">
            <div class="p-3 bg-orange-600 bg-opacity-75 rounded-full">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                class="bi bi-clipboard-data-fill w-8 h-8 text-white" viewBox="0 0 16 16">
                <path
            ...
            <div class="mx-5">
              <h4 class="text-2xl font-semibold text-gray-700">{{ Receipt }}</h4>
...
async function getCountUser() {
  try {
    const res = await httpService.getWithAuth(Statistic_API + '/count-user');
    User.value = res;
  }
  catch (error) {
    console.log(error);
...
async function getCountProduct() {
  try {
    const res = await httpService.getWithAuth(Statistic_API + '/count-product');
    Product.value = res;
  }
  catch (error) {
    console.log(error);
...
async function getCountReceipt() {
  try {
    const res = await httpService.getWithAuth(Statistic_API + '/count-receipt');
    Receipt.value = res;
...
async function getCountOrder() {
  try {
    const res = await httpService.getWithAuth(Statistic_API + '/count-order');
    Order.value = res;
  }
  catch (error) {
    console.log(error);
